...
profit_almonds_df_final <- profit_almonds_df %>%
  group_by(year) %>%
  summarise(mean_netprofit = mean(net))
...
mean_profit = cbind.data.frame(profit_almonds_df_final, parameters)

p1 = ggplot(mean_profit, aes(price, mean_netprofit, col=acres))+geom_point(cex=2)+
  labs(y="Mean Annual Profits", x="Annual Price")
p2 = ggplot(mean_profit, aes(acres, mean_netprofit, col=price))+geom_point(cex=2)+
